...
public class GhostSwap : MonoBehaviour
{
    ...
    {
        if (other.gameObject.name != "New Character2" &&
            other.gameObject.tag == "Player")// not required if plane or terrain does not have a collider enabled. had some issues with it activating trigger events.
        {
